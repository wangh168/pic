...
#define DDX_TEXT(nID, var) \
		...
		{ \
			DDX_Text(nID, var, sizeof(var), bSaveAndValidate);\
		}

#define DDX_TEXT_LEN(nID, var, len) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Text(nID, var, sizeof(var), bSaveAndValidate, TRUE, len); \
		}

#define DDX_INT(nID, var) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Int(nID, var, TRUE, bSaveAndValidate); \
		}

#define DDX_INT_RANGE(nID, var, min, max) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Int(nID, var, TRUE, bSaveAndValidate, TRUE, min, max); \
		}

#define DDX_UINT(nID, var) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Int(nID, var, FALSE, bSaveAndValidate); \
		}

#define DDX_UINT_RANGE(nID, var, min, max) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Int(nID, var, FALSE, bSaveAndValidate, TRUE, min, max); \
		}
...
#define DDX_FLOAT(nID, var) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Float(nID, var, bSaveAndValidate); \
		}

#define DDX_FLOAT_RANGE(nID, var, min, max) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Float(nID, var, bSaveAndValidate, TRUE, min, max); \
		}
        
#define DDX_FLOAT_P(nID, var, precision) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Float(nID, var, bSaveAndValidate, FALSE, 0, 0, precision); \
		}

#define DDX_FLOAT_P_RANGE(nID, var, min, max, precision) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			DDX_Float(nID, var, bSaveAndValidate, TRUE, min, max, precision); \
		}
#endif // _ATL_USE_DDX_FLOAT
